...
func buildNetwork(port uint16) (*Network, error) {
	...
	builder.AddComponentWithPriority(1, new(MockComponent))
...
func TestDuplicateComponent(t *testing.T) {
	...
	assert.Equal(t, builder.ComponentCount, 0, "should have no Components")

	err = builder.AddComponentWithPriority(1, new(MockComponent))
	assert.Equal(t, nil, err)
